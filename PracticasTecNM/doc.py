...
def _ListGcsFiles(gcs_file_pattern):
  ...
  # gs://bucket/file2
  # ...
  # The following list filtering eliminates the paths such as
  # `gs://bucket/directory/` when listing the directories using gsutil.
  return [x for x in output.splitlines() if len(os.path.basename(x))]
...
def UploadFiles(converted_files, target_gcs_directory):
  ...
  # Surround file paths with single quotes to handle whitespace and
  # other misc characters
  sanitized_files_to_upload = [
      "'" + src_path + "'" for src_path in files_to_upload
  ]
  cmd = 'gsutil -m cp {} {}'.format(' '.join(sanitized_files_to_upload),
                                    target_gcs_directory)
